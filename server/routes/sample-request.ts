...
// Log sample requests for tracking
const logSampleRequest = (data: any) => {
  console.log('📧 SAMPLE REQUEST RECEIVED:', {
    timestamp: new Date().toISOString(),
    name: data.name,
    email: data.email,
    company: data.company || 'Not provided',
    samplesOf: data.samplesOf,
    sentTo: 'info@elpdata.com'
  });
};

router.post('/api/sample-request', async (req, res) => {
  try {
    // Validate request body
    const validatedData = sampleRequestSchema.parse(req.body);
    
    // Log the sample request for tracking
    logSampleRequest(validatedData);
    ...
    const adminEmailOptions = {
      ...
      subject: '🔔 New Sample Request - ELP Data',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #ecfdf5; padding: 20px; border-radius: 8px; margin: 20px 0;">
            <h3 style="color: #374151; margin-top: 0;">Sample Request Details</h3>
            <p><strong>Samples Of:</strong></p>
            <div style="background-color: white; padding: 15px; border-radius: 4px; border-left: 4px solid #10b981;">
              ${validatedData.samplesOf}
            </div>
          </div>
          
          <div style="background-color: #eff6ff; padding: 15px; border-radius: 8px; margin: 20px 0;">
            <p style="margin: 0; color: #1e40af; font-weight: bold;">
              📧 URGENT: Respond within 24 hours with customized data samples
            </p>
    ...
    const userEmailOptions = {
      ...
      subject: 'Sample Request Confirmation - ELP Data',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #1e40af 0%, #10b981 100%); padding: 30px; border-radius: 8px 8px 0 0; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 28px;">Thank You, ${validatedData.name}!</h1>
            <p style="color: white; margin: 10px 0 0 0; font-size: 18px;">Your sample request has been received</p>
          </div>
          ...
          <div style="background-color: white; padding: 30px; border-radius: 0 0 8px 8px; border: 1px solid #e5e7eb;">
            ...
            <div style="display: flex; align-items: center; margin: 20px 0;">
              <div style="background-color: #dbeafe; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 15px;">
                <span style="color: #1e40af; font-weight: bold; font-size: 18px;">1</span>
              </div>
              <div>
                <h3 style="margin: 0; color: #374151;">Request Review (Within 2 hours)</h3>
                <p style="margin: 5px 0 0 0; color: #6b7280;">Our team will review your sample request and identify the best data sources</p>
              </div>
            </div>
            ...
            <div style="display: flex; align-items: center; margin: 20px 0;">
              <div style="background-color: #d1fae5; width: 40px; height: 40px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 15px;">
                <span style="color: #10b981; font-weight: bold; font-size: 18px;">2</span>
              </div>
              <div>
                <h3 style="margin: 0; color: #374151;">Sample Preparation (Within 24 hours)</h3>
                <p style="margin: 5px 0 0 0; color: #6b7280;">We'll prepare customized data samples matching your requirements</p>
              </div>
            </div>
            ...
            <div style="display: flex; align-items: center; margin: 20px 0;">
              ...
              <div>
                <h3 style="margin: 0; color: #374151;">Sample Delivery</h3>
                <p style="margin: 5px 0 0 0; color: #6b7280;">Receive your custom data samples via email with insights and recommendations</p>
              </div>
            </div>
            
            <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 30px 0;">
              <h3 style="color: #374151; margin-top: 0;">Your Request Summary</h3>
              <p><strong>Samples Of:</strong> ${validatedData.samplesOf}</p>
              ${validatedData.company ? `<p><strong>Company:</strong> ${validatedData.company}</p>` : ''}
              ${validatedData.bestTimeToReach ? `<p><strong>Best Time to Reach:</strong> ${validatedData.bestTimeToReach}</p>` : ''}
            </div>
            ...
            <div style="text-align: center; margin-top: 30px;">
              ...
              <p style="color: #1e40af; margin: 5px 0 0 0;">
                📧 <a href="mailto:info@elpdata.com" style="color: #1e40af;">info@elpdata.com</a> | 
                📞 <a href="tel:+13072242324" style="color: #1e40af;">(307) 224-2324</a>
              </p>
            </div>
          </div>
        </div>
    ...
    try {
      ...
      console.log('✅ Sample request notification sent to info@elpdata.com');
      ...
      console.log('📝 MANUAL FOLLOW-UP REQUIRED for sample request:', validatedData);
    }
